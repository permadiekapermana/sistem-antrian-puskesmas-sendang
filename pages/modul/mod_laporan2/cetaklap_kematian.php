...
$sampai=$_POST[sampai];
	...
	INNER JOIN `kematian` ON `kematian`.`id_penduduk` = `penduduk`.`id_penduduk` ORDER BY id_kematian DESC";
...
if (!empty($query))
	...
	$pdf->Cell(4,1,'Tempat, Tanggal Meninggal',1,0,'C');
...
while ($hasil=mysql_fetch_array($baca))
{
	...
	$pdf->Cell(4,1,$hasil['tempat_meninggal'],1,0,'C');
