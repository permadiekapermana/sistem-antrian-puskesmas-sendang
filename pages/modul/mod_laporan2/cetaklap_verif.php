...
$sampai=$_POST[sampai];
	...
	$query= "SELECT * FROM `penduduk_update` WHERE status_perbaikan='Valid'							  
			AND tgl_perbaikan Between '$dari' and '$sampai' ORDER BY id_update DESC";
...
if (!empty($query))
	...
	$pdf->Cell(8,1,'ID Verifikasi',1,0,'C');
	$pdf->Cell(5,1,'NIK Penduduk',1,0,'C');
	$pdf->Cell(8,1,'Nama Penduduk',1,0,'C');
	$pdf->Cell(4,1,'Tanggal Perbaikan',1,0,'C');
	$pdf->Cell(4,1,'Status Perbaikan',1,0,'C');
...
while ($hasil=mysql_fetch_array($baca))
{
	...
	$pdf->Cell(2,1,$no.'.',1,0,'C');
	$pdf->Cell(8,1,$hasil['id_update'],1,0,'L');
	$pdf->Cell(5,1,$hasil['nik'],1,0,'L');
	$pdf->Cell(8,1,$hasil['nama'],1,0,'L');
	$pdf->Cell(4,1,$hasil['tgl_perbaikan'],1,0,'C');
	$pdf->Cell(4,1,$hasil['status_perbaikan'],1,0,'C');
